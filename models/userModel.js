...
const createUser = async (userType, name, username, email, age, gender, passwordHash) => {
  ...
  try {
    ...
    if (userType === 'parent') {
      const connection = await connectToDatabase();
      const [result] = await connection.execute('INSERT INTO users(userType,name,username,email,age,gender,password) VALUES (?, ?, ?, ?, ?,?,?)', [userType, name, username, email, age, gender, passwordHash]);
      return { id: result.insertId }; // Returns the created user
    } else {
      return false;
